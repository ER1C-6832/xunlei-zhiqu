...
ContextKind = Literal["resource", "manual"]
JobContext = ResourceJobCreateRequest | ManualJobCreateRequest

_jobs: list[ResourceJobSnapshot] = []
_created_job_ids: set[str] = set()
_job_contexts: dict[str, tuple[ContextKind, JobContext]] = {}
_execution_requests: dict[str, DownloadExecutionRequest] = {}
_execution_statuses: dict[str, DownloadExecutionStatus] = {}
_history: list[LinkHistoryItem] = []
_persistence: DownloadStateStore | None = None
_store_initialized = False


def initialize_job_store(
    fixtures_enabled: bool = False,
    *,
    persistence: DownloadStateStore | None = None,
) -> None:
    """Initialize once, restoring local persisted jobs before optional fixtures."""
    global _store_initialized, _persistence
    ...
    if persistence is not None:
        for record in persistence.load_jobs():
            _restore_record(record)
        _jobs.sort(key=lambda item: item.created_at, reverse=True)
        _history.sort(key=lambda item: item.added_at, reverse=True)

    if fixtures_enabled:
        persisted_ids = {job.job_id for job in _jobs}
        jobs = [job for job in fixture_jobs() if job.job_id not in persisted_ids]
        _jobs.extend(jobs)
...
def _reset_job_store_for_tests(
    *,
    ...
    _execution_statuses.clear()
    _history.clear()
    _persistence = None
    _store_initialized = False
    initialize_job_store(fixtures_enabled, persistence=persistence)
...
def create_job(
    ...
    *,
    ...
    private_context: ResourceJobCreateRequest | None = None,
) -> ResourceJobSnapshot:
    for existing in _jobs:
        if existing.plan_id == payload.plan.plan_id and existing.delivery_target == payload.delivery_target:
            return existing

    selected_order = [candidate_id for item in payload.plan.selected for candidate_id in item.candidate_ids]
    selected_ids = set(selected_order)
    source_ids = selected_ids | {
        candidate_id for item in payload.plan.alternatives for candidate_id in item.candidate_ids
    }
    excluded_ids = {
        candidate_id for item in payload.plan.excluded for candidate_id in item.candidate_ids
    }
    total_bytes = total_bytes_override if total_bytes_override is not None else _selected_total_bytes(payload, selected_order)
    ...
    destination = destination_override or payload.destination or _default_destination(payload.plan.resource_title, target)
    ...
    _job_contexts[job.job_id] = ("resource", private_context or payload)
...
def create_manual_job(
    ...
    *,
    ...
    if job.execution_mode == "demo":
        _created_job_ids.add(job.job_id)
    _job_contexts[job.job_id] = ("manual", payload)
    _history.insert(
        0,
        LinkHistoryItem(
...
def set_favorite(history_id: str, favorite: bool) -> LinkHistoryItem | None:
    for index, item in enumerate(_history):
        if item.history_id != history_id:
            continue
        updated = item.model_copy(update={"favorite": favorite, "favorite_at": datetime.now(UTC) if favorite else None})
        _history[index] = updated
        if updated.job_id:
            _persist_job(updated.job_id)
        return updated
...
def project_execution_status(job_id: str, status: DownloadExecutionStatus) -> ResourceJobSnapshot | None:
    index = _find_index(job_id)
    if index is None:
        return None
    job = _jobs[index]
    ...
    next_action = {
        ...
        "paused": "resume",
        "completed": "open",
        "failed": "resume" if status.resume_available else None,
        "cancelled": None,
    }[status.state]
...
def persist_execution_state(
    ...
    """Persist executor state; `.part` size remains the actual resume offset truth."""
    job_id = request.job.job_id
    if _find_index(job_id) is None:
        return
    _execution_requests[job_id] = request
    _execution_statuses[job_id] = status
    project_execution_status(job_id, status)
    _persist_job(job_id)


def restored_execution_records() -> list[tuple[DownloadExecutionRequest, DownloadExecutionStatus]]:
    records: list[tuple[DownloadExecutionRequest, DownloadExecutionStatus]] = []
    for job in _jobs:
        request = _execution_requests.get(job.job_id)
        status = _execution_statuses.get(job.job_id)
        if request is not None and status is not None:
            records.append((request, status))
    return records


def get_private_job_context(job_id: str) -> tuple[ContextKind, JobContext] | None:
...
def cancel_job(job_id: str) -> bool:
    ...
    _execution_requests.pop(job_id, None)
    _execution_statuses.pop(job_id, None)
    ...
    for item in _history:
        if item.job_id != job_id:
            ...
            continue
        if item.favorite:
            retained_history.append(item.model_copy(update={"job_id": None, "delivery_target": None, "status": "saved"}))
    _history[:] = retained_history
    if _persistence is not None:
        _persistence.delete_job(job_id)
    return True
...
def _persist_job(job_id: str) -> None:
    ...
    if index is None:
        return
    context_entry = _job_contexts.get(job_id)
    context = None
    if context_entry is not None:
        kind, payload = context_entry
        context = {"kind": kind, "payload": payload.model_dump(mode="json")}
    execution_request = _execution_requests.get(job_id)
    execution_status = _execution_statuses.get(job_id)
    linked_history = [item.model_dump(mode="json") for item in _history if item.job_id == job_id]
    _persistence.upsert_job(job_id, {
        "schema_version": 1,
        "snapshot": _jobs[index].model_dump(mode="json"),
        "private_context": context,
        "execution_request": execution_request_to_dict(execution_request) if execution_request else None,
        "execution_status": execution_status_to_dict(execution_status) if execution_status else None,
...
def _restore_record(record: dict) -> None:
    try:
        snapshot = ResourceJobSnapshot.model_validate(record["snapshot"])
    except Exception:
        return
    if _find_index(snapshot.job_id) is not None:
        return
    _jobs.append(snapshot)
    context = record.get("private_context")
    if isinstance(context, dict):
        kind = context.get("kind")
        payload = context.get("payload")
        try:
            if kind == "resource":
                _job_contexts[snapshot.job_id] = ("resource", ResourceJobCreateRequest.model_validate(payload))
            elif kind == "manual":
                _job_contexts[snapshot.job_id] = ("manual", ManualJobCreateRequest.model_validate(payload))
        except Exception:
            pass
    try:
        if record.get("execution_request") is not None:
            _execution_requests[snapshot.job_id] = execution_request_from_dict(record["execution_request"])
        if record.get("execution_status") is not None:
            _execution_statuses[snapshot.job_id] = execution_status_from_dict(record["execution_status"])
    except Exception:
        _execution_requests.pop(snapshot.job_id, None)
        _execution_statuses.pop(snapshot.job_id, None)
    for item in record.get("history", []):
        try:
            history_item = LinkHistoryItem.model_validate(item)
        except Exception:
            continue
        if not any(existing.history_id == history_item.history_id for existing in _history):
...
def _record_history(job: ResourceJobSnapshot, payload: ResourceJobCreateRequest, selected_order: list[str]) -> None:
    ...
    for index, existing in enumerate(_history):
        if existing.display_link == display_link and existing.title == job.title and existing.job_id is None:
            _history[index] = existing.model_copy(update={
            ...
            return
    _history.insert(0, LinkHistoryItem(
        history_id=f"history_{uuid4().hex[:10]}",
        title=job.title,
        link_type=_link_type(raw_value, candidate.candidate_type if candidate else None),
        display_link=display_link,
...
def _advance_demo_job(job: ResourceJobSnapshot) -> ResourceJobSnapshot:
    if job.execution_mode != "demo":
        return job
    if job.job_id not in _created_job_ids or job.status in {"paused", "interrupted", "waiting_for_source", "completed"}:
        return job
    ...
    if job.status == "planning":
        ...
        return job.model_copy(update={
            ...
            "stage_label": "正在将资源保存到云盘（演示）" if cloud else "正在执行演示任务",
            "next_action": "pause",
        })
    ...
    if job.status == "downloading" and job.total_bytes == 0:
        return job.model_copy(update={"speed_bytes_per_second": 0, "eta_seconds": None, "stage_label": "已进入云盘任务队列" if cloud else "演示任务已创建"})
    return job
